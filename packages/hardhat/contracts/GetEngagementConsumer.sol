 ...
 */
contract GetEngagementConsumer is FunctionsClient, ConfirmedOwner, AutomationCompatibleInterface {
  using FunctionsRequest for FunctionsRequest.Request;

  string public twitterHandle;
  mapping(uint256 => bytes) public tweetIdsByDate;
  mapping(string => uint256) public engagementScores;
  ...
  function performUpkeep(bytes calldata) external override {
    ...
    bytes32 requestId = _sendRequest(performData, subscriptionId, fulfillGasLimit, donId);
  ...
  function setEngagementScore(string memory _twitterHandle, uint256 score) external onlyOwner {
    engagementScores[_twitterHandle] = score;
  }

  function _setEngagementScore(bytes memory engagementScore) internal {
    engagementScores[twitterHandles[engagementRequestIndex]] = abi.decode(engagementScore, (uint256));
   ...
   */
  function start(bytes calldata _tweetIds, string[] calldata _twitterHandles) external {
    require(isCalculatingEngagement == false, "Already calculating engagement");
    require(block.timestamp - startEngagementTimeStamp > 86400, "Already calculated engagement today");
