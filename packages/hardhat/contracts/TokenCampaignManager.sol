...
contract TokenCampaignManager is FunctionsClient, ConfirmedOwner, EIP712 {
	...
	constructor(
		...
		FunctionsClient(config.router)
		ConfirmedOwner(msg.sender)
		EIP712(_projectTwitterHandle, "1")
	{
		donId = config.donId;
	...
	function registerUser(
		...
		require(
			!isUserRegistered(_userData.twitterHandle),
			"Twitter handle is already registered"
		);
		// Register the user
	...
	function _setEngagementScore(bytes memory _engagementScore) internal {
		engagementScores["tomaspuricelli"] = _engagementScore;
	}
	...
	function isSameDay(
		...
		return
			getStartOfDayTimestamp(_timestamp1) ==
			getStartOfDayTimestamp(_timestamp2);
	}
	...
	function start() external {
		bytes memory tweetIds = getTweetsConsumerInstance.getTweetIdsByDate(
			block.timestamp
		);
	...
	function setEngagementScore(
		string calldata _twitterHandle,
		bytes memory _engagementScore
	) external {
		require(
			address(getEngagementConsumerInstance) == msg.sender,
			"Only Chainfluence can set the engagement score"
		);
		engagementScores[_twitterHandle] = _engagementScore;
	}

	function requestTweets() public {
